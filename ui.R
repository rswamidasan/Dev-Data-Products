    ...
    sidebarLayout(
        sidebarPanel(
            ...
            h5("The Central Limit Theorem (CLT) states that when a distribution with mean(mu)
                and finite variance (Var) is randomly sampled, the mean of the samples is normally
                distributed with mean = mu and variance = Var/N, when the sample size (N), is
                sufficiently large."),
            ...
            p(h5("Use the Explorer to examine* that for a given population with parameters mu and Var:")),
            p(h5(tags$ol(tags$li("The Sample Means Variance varies inversely as Sample Size."),
                         ...
                         tags$li("The Diminishing Returns of Sample Size: as Sample Size increases,
                                    the reduction in the Standard Error of the Sample Mean decreases.")))),

            p(h5("The Source Code for this app is available at the author's ",
                 (a (href = "https://github.com/rswamidasan/Dev-Data-Products",
                                target="_blank", "GitHub repo.")))),

            # Remove, as it doesn't do much on shiny apps io server
